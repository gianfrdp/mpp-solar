...
log = logging.getLogger("hassd2_mqtt")


class hassd2_mqtt(mqtt):
    def __str__(self):
    ...
    def output(self, *args, **kwargs):
        ...
        log.info("Using output processor: hassd2_mqtt")
        log.debug(f"kwargs {kwargs}")
        data = get_kwargs(kwargs, "data")
        # exit if no data
        if data is None:
        ...
        log.debug(f"hassd2_mqtt.output config_msgs {config_msgs}")
        log.debug(f"hassd2_mqtt.output value_msgs {value_msgs}")
